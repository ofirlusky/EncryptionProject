...
public class BoardPanel extends JPanel {
    ...
    private void updateStatusMessage() {
        if (game.isGameOver()) {
            String winner = (game.getCurrentTurn() == PlayerID.PLAYER_ONE) ? "Player 1 (Blue)" : "Player 2 (Red)";
            statusLabel.setText("game over" + winner + " win");
            statusLabel.setForeground(Color.YELLOW);
            return;
        }
        String playerColor = (game.getCurrentTurn() == PlayerID.PLAYER_ONE) ? "Player 1 (Blue)" : "Player 2 (Red)";
        String phaseText;
